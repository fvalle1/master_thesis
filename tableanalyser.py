...
def geneinfo(genename, df, nfiles, metric='fpkm'):
    ...
    if 'fpkm' in metric:
        genedata = np.array(
            [fpkm for fpkm in df.loc[df['gene'] == genename].loc[:, df.keys()[1:]].values.reshape(nfiles, 1) if
             (fpkm > 1e-1) & (fpkm < 1e5)])
    else:
...
def genedistr(genedict, bins=50, ax=None, density=False, label='', save=True, metric='fpkm', logx=True, logy=True):
    ...
    if ax == None:
        ...
        fig = ax.get_figure()
    n, bin_edges, _ = ax.hist(genedict['data'], lw=1.5, density=density, histtype='step', range=_range, bins=bins,
                              label=label)
...
def genecoord(genedict, means, variances, metric='fpkm'):
    ...
    plt.scatter([np.average(genedict['data'])], [np.var(genedict['data'])], marker='x', c='r', s=90,
                label=genedict['name'])
...
def discretize(column, nquartiles=10):
    quantiles = np.quantile(column, np.linspace(0, 1, nquartiles))
...
def plotvarmen(means, variances, ax=None, normalisation_str="counts"):
    x_lin = np.logspace(np.log10(means[means.nonzero()].min()), np.log10(means[means.nonzero()].max()), dtype=float,
                        num=50)
    if ax is None:
...
def plotoversigmacv2(means, variances, ax=None, normalisation_str="counts", how_many_sigmas=3):
    ...
    ax.plot((bin_edges[:-1] + bin_edges[1:]) / 2, bin_means + bin_sigmas * how_many_sigmas, lw=3, color='yellow',
...
def plotoverpoints(means, variances, over_plot, ax=None, normalisation_str="counts", how_many_sigmas=3):
    ...
    bin_means, bin_edges, _ = st.binned_statistic(means[means.nonzero()], cv2, statistic='median', bins=log_bins_for_x)
    bin_sigmas, _, _ = stats.binned_statistic(means[means.nonzero()], cv2, statistic=np.std, bins=log_bins_for_x)
    ax.hlines(bin_means + bin_sigmas * how_many_sigmas, bin_edges[1:], bin_edges[:-1], lw=3, color='yellow',
              label='binned average + $%d\sigma$' % how_many_sigmas)
...
def getovergenes(df_mv, func, method='sampling', distance=10, how_many_sigmas=3, knee=100):
    ...
    df_over = pd.DataFrame(index=df_mv.index, columns=['distance'])
    for g in df_mv.index:
        ...
        if 'sampling' in method:
            ...
            r = func(mean, float(var) / mean / mean, bin_means=bin_means, bin_sigmas=bin_sigmas, bin_edges=bin_edges,
...
def get_mean_cv2_sigma(mean, cv2, how_many_sigmas=3):
    ...
    for i in range(len(bin_edges[:-1])):
        if mean <= bin_edges[i + 1] and mean > bin_edges[i]:
            ...
            break
    return (
    mean, cv2, bin_means[bin_i], bin_sigmas[bin_i], cv2 > (bin_means[bin_i] + how_many_sigmas * bin_sigmas[bin_i]))


def scalinglawsandoverexpressed(working_dir, normalisation_str="counts", method='sampling', how_many_sigmas=3,
                                distance=10):
